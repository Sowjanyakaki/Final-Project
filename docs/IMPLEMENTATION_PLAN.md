...
- ~~Transit/amenities were always shown as "unavailable"~~ — resolved: `GET /api/shortlist` hardcoded `transit: []`/`amenities: []`/`uncertain: true` regardless of OSM MCP's status, a leftover from before that integration was deployed and working. The user asked whether the app surfaces area/neighborhood info and this gap surfaced in answering — safety was already real (RAG/Wikipedia) but transit/amenities weren't, even though the underlying OSM tool has worked since earlier this session. Now calls `osmNearby` for both categories per listing (when it has coordinates — all 187 scraped listings do), same tool the voice agent already uses, with real OSM citations added. TDD'd (both the coordinates and no-coordinates paths) and live-verified: confirmed the real Railway OSM service returns real POIs for some listings (proving the wiring works), and correctly, honestly reports "unavailable" for others whose exact scraped coordinates have no OSM-indexed points nearby (verified directly against the real API — genuinely empty, not a bug) — hit rate depends on OSM's real-world data density at each listing's specific coordinate, which the app has no control over and doesn't paper over.
