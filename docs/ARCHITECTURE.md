...
| LLM access | Groq via AI SDK's Groq provider (`@ai-sdk/groq`), default model `openai/gpt-oss-120b` (tool-calling support required for the agent; `llama-3.3-70b-versatile`/`llama-3.1-8b-instant` were retired by Groq and no longer resolve) | User-provided Groq API key; Groq's inference speed suits a voice agent where response latency matters |
